...
def operacion():
    ...
    if opcion.get() == 1:
        ...
        total = numero*numero
    print("El total de la operacion es : %i." % total)


ventana = Tk()
opcion = IntVar()
num = IntVar()
ventana.title("Radiobutton es tkinter")
ventana.geometry("400x300")
Label(ventana,text="Escribe el número: ").place(x=20,y=20)
Entry(ventana, textvariable=num).place(x=130, y=20)
Label(ventana, text="Escribe tu opción: ").place(x=20, y=50)
...
Button(ventana,text='Realiza operación', command=operacion).place(x=20, y=140)
ventana.mainloop()
